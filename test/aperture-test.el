...
(defmacro aperture-test--with-fake-frame-make (make-frame &rest body)
  "Run BODY with MAKE-FRAME as `make-frame' over a fake 1000x800 px parent."
  (declare (indent 1) (debug t))
  `(let ((aperture-child-frame-width 0.5)
         (aperture-child-frame-height 0.5)
         (aperture-child-frame-position 'center))
     (cl-letf (((symbol-function 'frame-pixel-width) (lambda (&rest _) 1000))
               ...
               ((symbol-function 'frame-char-width) (lambda (&rest _) 10))
               ((symbol-function 'frame-char-height) (lambda (&rest _) 20))
               ((symbol-function 'frame-native-width) (lambda (&rest _) 120))
               ((symbol-function 'frame-text-width) (lambda (&rest _) 100))
               ((symbol-function 'frame-native-height) (lambda (&rest _) 60))
               ((symbol-function 'frame-text-height) (lambda (&rest _) 50))
               ((symbol-function 'make-frame) ,make-frame)
...
and the frame was never deleted.  Regression test for that."
  ...
  (let ((after-make-frame-functions (list #'select-frame))
        (hooks-at-make 'unset))
    (aperture-test--with-fake-frame-make
        (lambda (&rest _)
...
(ert-deftest aperture-test-child-frame-blend ()
  (require 'aperture-child-frame)
  (let ((black '(0 0 0))
        (white '(65535 65535 65535)))
    (should (equal (aperture-child-frame--blend black white 0.0) "#000000"))
    (should (equal (aperture-child-frame--blend black white 1.0) "#ffffff"))
    (should (equal (aperture-child-frame--blend black white 0.25) "#404040"))
    (should (equal (aperture-child-frame--blend white black 0.25) "#bfbfbf"))))

(ert-deftest aperture-test-child-frame-background-forms ()
  (require 'aperture-child-frame)
  (let ((aperture-child-frame-background-blend 0.25)
        (colors '(("dark" . (0 0 0)) ("light" . (65535 65535 65535))))
        bg fg)
    (cl-letf (((symbol-function 'face-background) (lambda (&rest _) bg))
              ((symbol-function 'face-foreground) (lambda (&rest _) fg))
              ((symbol-function 'color-values)
               (lambda (c &rest _) (cdr (assoc c colors)))))
      (let ((aperture-child-frame-background 'auto))
        (setq bg "dark" fg "light")
        (should (equal (aperture-child-frame--background nil) "#404040"))
        (setq bg "light" fg "dark")
        (should (equal (aperture-child-frame--background nil) "#bfbfbf"))
        (setq bg nil fg nil)
        (should-not (aperture-child-frame--background nil))
        (setq bg "unspecified-bg" fg "unspecified-fg")
        (should-not (aperture-child-frame--background nil)))
      (let ((aperture-child-frame-background nil))
        (should-not (aperture-child-frame--background nil)))
      (let ((aperture-child-frame-background "#123456"))
        (should (equal (aperture-child-frame--background nil) "#123456"))))))

(ert-deftest aperture-test-child-frame-make-passes-background ()
  "A face styled with a color of its own keeps it."
  (require 'aperture-child-frame)
  (let ((aperture-child-frame-parameters nil)
        (aperture-child-frame-background "#202020")
        params faces)
    (aperture-test--with-fake-frame-make
        (lambda (p) (setq params p) 'fake-frame)
      (cl-letf (;; Only `child-frame-border' is styled.
                ((symbol-function 'face-background)
                 (lambda (face &rest _)
                   (if (eq face 'child-frame-border) "#abcdef" "#000000")))
                ((symbol-function 'color-values) (lambda (c &rest _) c))
                ((symbol-function 'set-face-background)
                 (lambda (face color frame)
                   (should (eq frame 'fake-frame))
                   (setf (alist-get face faces) color))))
        (aperture-child-frame--make nil)
        (should (equal (alist-get 'background-color params) "#202020"))
        (should (equal faces '((internal-border . "#202020")
                               (fringe . "#202020")
                               (default . "#202020"))))
        (let ((aperture-child-frame-parameters '((background-color . "#ff0000"))))
          (setq faces nil)
          (aperture-child-frame--make nil)
          (should (equal (alist-get 'background-color params) "#ff0000"))
          (should (equal faces '((internal-border . "#ff0000")
                                 (fringe . "#ff0000")
                                 (default . "#ff0000")))))
        (let ((aperture-child-frame-background nil))
          (setq faces nil)
          (aperture-child-frame--make nil)
          (should-not (assq 'background-color params))
          (should-not faces))))))

(ert-deftest aperture-test-child-frame-make-sizes-the-whole-frame ()
  "The border and fringes come out of the geometry's size."
  (require 'aperture-child-frame)
  (let ((aperture-child-frame-background nil)
        size)
    (aperture-test--with-fake-frame-make (lambda (&rest _) 'fake-frame)
      (cl-letf (((symbol-function 'set-frame-size)
                 (lambda (_frame w h &optional _pixelwise) (setq size (list w h)))))
        (aperture-child-frame--make nil)
        ;; 500x400 from the geometry, less 20 across and 10 down.
        (should (equal size '(480 390)))))))
